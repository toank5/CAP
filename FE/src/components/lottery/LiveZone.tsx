    ...
    <section className="space-y-4 rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 dark:border-blue-800 dark:from-blue-950/40 dark:to-indigo-950/40">
      ...
      <div>
        ...
        {remaining > 0 && (
          <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
            Còn lại: <strong className="text-amber-600 dark:text-amber-400">{remaining} căn</strong> · Tỷ lệ trúng: <strong className="text-emerald-600 dark:text-emerald-400">{state?.winRatePercentage ?? 0}%</strong>
          </p>
        )}
      </div>
      ...
      <div className="rounded-xl border border-blue-200 bg-white/80 py-6 text-center dark:border-blue-700 dark:bg-slate-900/60">
        {state?.nextCandidate ? (
          ...
          <div className="space-y-1">
            <p className="text-2xl">⏳</p>
            <p className="font-medium text-slate-600 dark:text-slate-300">Chờ CĐT bắt đầu bốc</p>
          </div>
        )}
      </div>

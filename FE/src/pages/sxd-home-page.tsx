  ...
  return (
    <div className="w-full space-y-8 pb-12">
      ...
      <motion.div
        ...
        className="rounded-3xl border border-slate-200/80 bg-white/90 p-5 sm:p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/90 space-y-4"
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
              <Building2 className="h-5 w-5 text-amber-500" />
              Dự án Nhà ở Xã hội trên địa bàn TP.HCM
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Giám sát quy hoạch, pháp lý và tiến độ cung ứng căn hộ của các chủ đầu tư
            </p>
          </div>
          <Button
            variant="outline"
          ...
          >
            Quản lý tất cả dự án <ArrowRight className="ml-1.5 h-3.5 w-3.5" />
          </Button>
        </div>
        ...
        {loading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="rounded-2xl border border-slate-100 p-4 dark:border-slate-800">
                <div className="aspect-[16/10] w-full animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-800" />
                <div className="mt-3 h-4 w-3/4 animate-pulse rounded bg-slate-100 dark:bg-slate-800" />
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 py-10 text-center dark:border-slate-800">
            <Building2 className="mx-auto h-10 w-10 text-slate-400 opacity-60" />
            <p className="mt-2 text-sm font-semibold text-slate-700 dark:text-slate-300">
              Chưa có dự án nào được khởi tạo trên hệ thống
            </p>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {data.projects.slice(0, 6).map((project) => {
              const isApproved = String(project.status || '').toUpperCase() === 'APPROVED' || String(project.status || '').toUpperCase() === 'OPEN'
              ...
              return (
                <div
                  key={project.id}
                  className="group flex flex-col justify-between rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4 transition-all hover:border-amber-400 hover:bg-white hover:shadow-md dark:border-slate-800 dark:bg-slate-850/50 dark:hover:bg-slate-800"
                >
                  <div className="space-y-3">
                    ...
                    <div className="relative aspect-[16/10] w-full overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/50">
                      {thumb ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-amber-900/30 to-slate-850 text-amber-300">
                    ...
                    <div>
                      <h4 className="line-clamp-1 text-sm font-bold text-slate-900 group-hover:text-amber-600 dark:text-white dark:group-hover:text-amber-400">
                        {project.projectName || (project as any).name}
                      </h4>
                      <p className="line-clamp-1 text-xs text-slate-500 mt-0.5">
                        📍 {project.location || (project as any).address || 'TP. Hồ Chí Minh'}
                      </p>
                    </div>

                    <div className="flex items-center justify-between rounded-xl bg-slate-100/80 px-3 py-2 text-xs dark:bg-slate-800/80">
                      <span className="text-slate-500 font-medium">Quỹ căn phê duyệt</span>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => navigate('sxd-projects')} className="flex-1 rounded-xl text-xs font-semibold h-8">
                      <Eye className="mr-1 h-3 w-3" /> Chi tiết
                    </Button>
                    <Button size="sm" onClick={() => navigate('applications')} className="flex-1 rounded-xl bg-amber-600 text-xs font-semibold text-white h-8 hover:bg-amber-700">
                      <FileText className="mr-1 h-3 w-3" /> Hồ sơ nộp
                    </Button>
                  </div>
                </div>
              )

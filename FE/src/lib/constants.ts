...
export function formatPriorityGroup(code: string | null | undefined): string {
  ...
  if (key.includes('LOW_INCOME') || key.includes('THU_NHAP_THAP')) return 'Người thu nhập thấp tại đô thị'
  if (key.includes('MERIT') || key.includes('NGUOI_CO_CONG')) return 'Người có công với cách mạng'
  return code
}
